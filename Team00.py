...
class Player:
    ...
    def play_fox(self, board):
        ...
        print("Fox is in position (" +
              str(fox_pos[0]) + "," + str(fox_pos[1]) + ")")
        while cont:
            new_pos = [int(i) for i in input(
                "Give the new position for the fox (row,column): ").split(',')]
            move.append(new_pos)
    ...
    def play_goose(self, board):
        ...
        goose_pos = [int(i) for i in input(
            "Give the position of the goose to move (row,column): ").split(',')]

        move = [goose_pos]

        # Get player input for goose next position
        new_pos = [int(i) for i in input(
            "Give the new position (row,column): ").split(',')]
        move.append(new_pos)
